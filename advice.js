...
const move = ['Move to Miami', 'Move to New York', 'Move to Morocco', 'Move to Tokyo', 'Move to Thailand', 'Move to Mexico', 'Move to Brazil', 'Move to Vegas', 'Move to Hollywood', 'Move to Moscow', 'Move to San Francisco', 'Move to Egypt', 'Move to The Bahamas', 'Move to Jamaica' ]
...
const chosenCats = randCats(categoryList)


//takes a category array and selects one random item
const findItemInList = (list)=>{
const userItem = list[Math.floor(list.length*Math.random())]
return userItem 
}
...
const userEvents = getItemsFromChosenCategories(chosenCats)

//printing the final message of excellent life changing advice
